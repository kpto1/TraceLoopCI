...
2. **流式代理在 Nginx 后缓冲**：如果前面挂了 Nginx，记得加 `proxy_buffering off;` 和 `proxy_cache off;`，否则 stream 会在 Nginx 那卡住
3. **LLM-as-Judge 打分有波动**：裁判模型打分不是绝对稳定的，建议设阈值（比如 >=6/10 算通过），不要和基线分数严格比较
4. **只支持 OpenAI 兼容格式**：非标准协议的模型（比如某些自建 API）需要自己写适配层。OpenAI 格式的都能接
5. **单机部署够用**：万级日调用量没问题。再往上建议分片部署：多个 collector -> 同一个 Redis + PostgreSQL
6. **评测跑得慢**：每个 case 都调一次 LLM，100 个 case 就要调 100 次。建议把基准线结果缓存起来，PR 只对比 diff
7. **Mock LLM 极简**：内置 mock 只有关键词匹配，不支持复杂对话逻辑。要测试真实场景就配真 API Key
